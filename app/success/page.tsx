...
function Confetti() {
  ...
  useEffect(() => {
    const colors = ["#1d4ed8", "#3b82f6", "#f59e0b", "#3b82f6", "#8b5cf6", "#06b6d4"];
      ...
      <div className="max-w-lg w-full mx-auto px-4">
        ...
        <div className="space-y-4 mb-8">
          ...
          <Link href="/tool" className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-400 hover:shadow-md transition-all group">
            <div className="w-10 h-10 bg-slate-950 text-white rounded-full flex items-center justify-center font-bold text-lg shrink-0 group-hover:bg-blue-700">1</div>
            <div className="flex-1">
              <p className="font-bold text-gray-800 text-sm">今抱えている医療クレーム状況を入力する</p>
              <p className="text-xs text-gray-400">種別・深刻度を選ぶだけで医療対応文が完成</p>
            </div>
            <span className="text-gray-300 group-hover:text-blue-600 transition-colors">→</span>
          </Link>

          <Link href="/tool" className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-400 hover:shadow-md transition-all group">
            <div className="w-10 h-10 bg-slate-950 text-white rounded-full flex items-center justify-center font-bold text-lg shrink-0 group-hover:bg-blue-700">2</div>
            <div className="flex-1">
              <p className="font-bold text-gray-800 text-sm">深刻度「重度」で毅然とした医療機関向け断り文を確認</p>
              <p className="text-xs text-gray-400">法的根拠を含む医療特化の対応文</p>
            </div>
            <span className="text-gray-300 group-hover:text-blue-600 transition-colors">→</span>
          </Link>

          <Link href="/tool" className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-400 hover:shadow-md transition-all group">
            <div className="w-10 h-10 bg-slate-950 text-white rounded-full flex items-center justify-center font-bold text-lg shrink-0 group-hover:bg-blue-700">3</div>
            <div className="flex-1">
              <p className="font-bold text-gray-800 text-sm">インシデントレポートを活用する</p>
              <p className="text-xs text-gray-400">行政報告・保険会社報告に備えた記録管理</p>
            </div>
            <span className="text-gray-300 group-hover:text-blue-600 transition-colors">→</span>
          </Link>
        </div>
        ...
        <div className="text-center bg-gray-50 rounded-xl p-4 border border-gray-100 mb-6">
          <p className="text-xs text-gray-500 mb-1">医療クレームが発生したらすぐアクセス</p>

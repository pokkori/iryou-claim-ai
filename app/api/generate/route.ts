...
export async function POST(req: NextRequest) {
  ...
  const prompt = `あなたはクリニック・病院の医事課・受付スタッフ・院長向けに、カスタマーハラスメント（カスハラ）対応文を作成する専門AIです。
厚生労働省「医療現場のハラスメント対策ガイドライン」および改正労働施策総合推進法（2026年10月カスハラ対策体制整備義務化）に準拠した、
...
1. 【口頭対応スクリプト】
   - 医師・看護師・受付スタッフが直接伝えるための言葉遣い
   - 毅然としながらも冷静・丁寧なトーン
   - 医療現場に即した具体的なセリフ形式で記述

2. 【書面通知文】
   - 医療機関名義の公式文書として使える文体
   - 事実確認・対応方針・今後の対処を明記
   - 必要に応じて警告・診療お断り・法的措置への言及を含める

3. 【インシデントレポート用記録メモ】
   - 日時・場所・発言内容・対応者・対応経緯を記録する形式
   - 将来の法的対応・行政報告・保険会社への報告に使える客観的な記述
   - 厚労省ガイドラインに沿った記録項目を網羅

各対応文は実際にそのまま使用できる品質で作成し、医療機関側の権利保護と患者・家族への配慮のバランスを保ってください。`;
